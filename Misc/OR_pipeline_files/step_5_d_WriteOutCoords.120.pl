...
my $foldername='./';
############ YOU HAVE TO SET THIS VALUE and ENTER FILE NAME FOR EACH SPP SEPARATELY ###########
my $first_tm=48;
# elut: 268
# mfur: 298
# elut2: 296
...
# come before your first domain. 

opendir(FOLDER,"$foldername") or die print "can not find the file";
my @array = grep(/step_4_result_mafftAlignment_removed.fasta$/,readdir(FOLDER));
close FOLDER;
...
my $input=$filename;
my $output="step_5_result.pickedM.CoordinatesOfBestStart.txt";
...
my @array; 
	...
	{ 
		...
		#print OUT $Nsequence;
		#print OUT $part2,"\n\n";
		# This is the start coordinate in the protein (anything before it gets cut off)
		# Note that perl is ZERO BASED
		print OUT ($length-$best_start), "\n";
		next;}
